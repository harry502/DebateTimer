module core {
	export class LoadingManage implements IBasicObject {
		private static _create: boolean;
		private static _instance: LoadingManage;
		public static getInstance(): LoadingManage {
			if (LoadingManage._instance == null) {
				LoadingManage._create = true;
				LoadingManage._instance = new LoadingManage();
				LoadingManage._create = false;
			}
			return LoadingManage._instance;
		}
		...
		public constructor() {
				if (!LoadingManage._create) {
					Log.error("LoadingManage 单例类，不能通过构造函数实例化。请使用 LoadingManage.getInstance() 方法访问该实例");
					return;
				}
		...
		private onResourceLoadError(event: RES.ResourceEvent): void {
			Log.warning(event.groupName + "资源组有资源加载失败");
